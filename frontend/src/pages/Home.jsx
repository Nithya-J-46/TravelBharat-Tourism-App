...
import { Search, MapPin, Compass, ShieldCheck, Landmark, ArrowRight, Star, Heart, Navigation, Calendar } from 'lucide-react';
...
const Home = () => {
    ...
    const loadData = async () => {
      try {
        const [statesRes, trendingRes, gemsRes, weekendRes, allPlacesRes] = await Promise.all([
          ...
          axios.get('http://localhost:5000/api/places?recommendationType=trending'),
          axios.get('http://localhost:5000/api/places?recommendationType=hidden-gems'),
          axios.get('http://localhost:5000/api/places?recommendationType=weekend'),
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:5000/api/places/suggestions?q=${encodeURIComponent(searchQuery)}`);
  ...
  return (
    <div className="relative">
      ...
      {/* Hero Section */}
      <div 
        className="relative h-[620px] flex items-center justify-center bg-cover bg-center"
      ...
      >
        ...
        <div className="relative z-10 text-center px-4 w-full max-w-4xl space-y-6">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-extrabold text-white tracking-tight leading-none drop-shadow-md">
            Discover the <span className="text-sky-300">Soul</span> of India
          </h1>
          ...
          {/* Autocomplete Search */}
          <div ref={suggestionRef} className="relative w-full max-w-2xl mx-auto mt-8">
            <form 
              onSubmit={handleSearchSubmit} 
              className="flex items-center bg-white/95 backdrop-blur-sm rounded-2xl p-2 shadow-2xl border border-white"
            >
              <div className="flex-grow flex items-center pl-4">
                <Search className="h-5 w-5 text-indigo-600 flex-shrink-0" />
                <input 
                  type="text" 
                  placeholder="Where do you want to explore? (e.g. Kashmir, Taj Mahal, temple)" 
                  className="w-full py-3 px-3 text-slate-800 focus:outline-none placeholder-slate-400 font-bold text-sm sm:text-base bg-transparent"
                  value={searchQuery}
                  onChange={(e) => {
              ...
              <button 
                type="submit"
                className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 shadow-md cursor-pointer hover:shadow-lg text-sm sm:text-base flex-shrink-0"
              >
                Search
              </button>
            </form>
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl border border-slate-100 shadow-2xl overflow-hidden z-40 text-left animate-fadeIn">
                {suggestions.map((sug, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleSuggestionClick(sug)}
                    className="w-full px-5 py-3.5 hover:bg-slate-50 transition border-b border-slate-50 last:border-0 flex items-center justify-between text-slate-700 font-semibold text-sm cursor-pointer"
                  >
